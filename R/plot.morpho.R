  ...
  if (reconstructed) {
    if (length(b.cols) == 2) {
      for (p in 1:length(b.cols[[2]])) {
        ...
        tree.age    <- max(ape::node.depth.edgelength(data$trees$TimeTree))
        child_node  <- data$trees$EvolTree$edge[branch, 2]
        time_branch <- which(data$trees$TimeTree$edge[, 2] == child_node)
        
        time <- fossil_pos
        actual_position <- if (root.edge) {
          tree.age - time + data$trees$TimeTree$root.edge
        } else {
          tree.age - time
        }
        
        position <- actual_position / data$trees$TimeTree$edge.length[time_branch]
        point_x  <- position * (edge_end_x[branch] - edge_start_x[branch])
        y_vals   <- yy[child_node]
        
        segments(point_x, y_vals, edge_end_x[branch], y_vals, col = "white")
        segments(point_x, y_vals, edge_end_x[branch], y_vals, col = "grey")
      }
    }
  }
